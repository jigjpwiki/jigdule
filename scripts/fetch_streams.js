...
function generateHTML(events, streamers) {
  ...
  const dates = Object.keys(groups).sort().reverse(); // 新しい日付順でも可
...
(async()=>{
  try {
    ...
    for (const s of list) {
      ...
      (await fetchTwitchVods(s.twitchUserLogin, token)).forEach(v=>events.push(v));
      try {
        (await fetchYouTube(s.youtubeChannelId,'order=date')).forEach(y=>events.push(y));
      } catch {}
    }
